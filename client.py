...
class MilvusClient(object):
    ...
    def create_collection(self, dimension, segment_row_limit, data_type=DataType.FLOAT_VECTOR, auto_id=False, collection_name=None, other_fields=None):
    ...
    def generate_entities(self, vectors, ids=None, collection_name=None):
        ...
        for field in info["fields"]:
            ...
            entities.append({"name": field["name"], "type": field_type, "values": self.generate_values(field_type, vectors, ids)})
        return entities
    ...
    def create_index(self, field_name, index_type, metric_type, index_param=None):
        ...
        logger.info("Building index start, collection_name: %s, index_type: %s, metric_type: %s" % (self._collection_name, index_type, metric_type))
        if index_param:
