...
class AsyncSearchRunner(BaseRunner):
    """run insert and search"""
    name = "async_search_performance"

    def __init__(self, env, metric):
        super(AsyncSearchRunner, self).__init__(env, metric)
        self.build_time = None
        self.insert_result = None
    ...
    def prepare(self, **case_param):
        ...
        if build_index is True:
            if case_param["index_type"]:
                ...
                build_index = False
                logger.warning("Please specify the index_type")
        insert_result = self.insert(self.milvus, collection_name, case_param["data_type"], dimension,
